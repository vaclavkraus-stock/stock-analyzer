    ...
    try{
      ...
      const res = await fetch("/api/analyze", {
        ...
        body:JSON.stringify({
          ...
          messages:[{role:"user",content:`Search web for "${t}" stock on ${exchInfo.label} (${exchInfo.hint}). Currency: ${exchInfo.currency}. Today: March 2026. Do 3 searches: 1) "${t} ${exchInfo.hint} stock price analyst target 2026" 2) "${t} stock news March 2026" 3) "${t} news site:reuters.com OR site:bloomberg.com OR site:cnbc.com OR site:finance.yahoo.com OR site:marketwatch.com OR site:investing.com". Return ONLY raw JSON, no markdown. ALL prices in ${exchInfo.currency}. CRITICAL: radarScores must be REAL scores 1-10 based on actual fundamentals (NOT all 5). investmentThesis must be 8-10 sentences in Czech. newsSummary = 6-8 sentences Czech summary. CRITICAL for news urls: copy the EXACT url from search results - only use urls you actually found in search results, never invent urls.
